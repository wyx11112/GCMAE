...
def load_large_dataset(dataset_name):
    ...
    return dataset, graph, label, split_idx
...
def load_dataset(dataset_name):
    ...
    if dataset_name.startswith("ogbn"):
    ...
    elif dataset_name in ['computer', 'photo', 'cs']:
        graph = dataset[0]
        graph = graph.remove_self_loop()
        graph = graph.add_self_loop()
        node_label = graph.ndata['label'].view(-1)
        sample_num = len(node_label)
        train_idx = range(int(1 * sample_num))
        val_idx = range(int(0.2 * sample_num))
        test_idx = range(int(1 * sample_num))
        train_mask = sample_mask(train_idx, sample_num)
        val_mask = sample_mask(val_idx, sample_num)
        test_mask = sample_mask(test_idx, sample_num)
        ...
        graph = graph.add_self_loop()
        node_label = graph.ndata['label'].view(-1)
        num_features = node_label.max().item() + 1
        num_classes = num_features
        graph.ndata['feat'] = F.one_hot(node_label, num_classes=num_classes).float()
        sample_num = len(node_label)
        train_idx = range(int(1 * sample_num))
        val_idx = range(int(0.2 * sample_num))
        test_idx = range(int(1 * sample_num))
        ...
        # feature_dim = degrees.max().item() + 1
